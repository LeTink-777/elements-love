...
async function deliverReport(
  ...
  try {
    ...
    const pdfBuffer = await generatePDF({
      title: "Ваша совместимость по стихиям",
      userName: subtitle,
      sections,
      siteName: SITE_NAME,
    });
    ...
    await sendResultEmail({
      to: email,
      subject: "Ваш разбор совместимости стихий готов",
      userName: subtitle,
      resultHtml: sections
        .map(
          (section) =>
            `<h3 style="color:#FF4D00;font-size:17px;margin:24px 0 8px;">${section.title}</h3>` +
            `<p style="font-size:15px;line-height:1.6;margin:0;white-space:pre-line;">${section.content}</p>`
        )
        .join(""),
      pdfBuffer,
      fileName: "sovmestimost-stihiy.pdf",
      siteName: SITE_NAME,
    });
