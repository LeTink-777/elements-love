...
export async function sendResultEmail({
  ...
  const { data, error } = await resend().emails.send({
    ...
    subject,
    html: `
      <div style="font-family: Georgia, serif; max-width: 600px; margin: 0 auto; padding: 40px 20px; background: #0A0A0F; color: #EDEDF2;">
        <h1 style="color: #FF4D00; font-size: 24px; margin-bottom: 20px;">${escapeHtml(siteName)}</h1>
        ...
        ${resultHtml}
        <hr style="border: 1px solid #14141C; margin: 30px 0;" />
